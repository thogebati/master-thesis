...
def generate_image_by_prompt(prompt, output_path, file_name, save_previews=False):
  """
  ...
  Args:
    ...
    file_name (str): the name of the final image file.
  ...
  try:
    ...
    images = track_progress(prompt, ws, prompt_id)
    # images = get_images(prompt_id, server_address, save_previews)
    save_image(images, output_path, file_name, save_previews)
  finally:
    ws.close()
...
def save_image(images, output_path, file_name, save_previews):
  """
...
def track_progress(prompt, ws, prompt_id):
  ...
  pbar_ksampler = None
  pbar_task = tqdm(enumerate(range(len(node_ids))), total=len(node_ids), position=1, leave=True, desc="comfy steps")
  ...
  while True:
      ...
      if isinstance(out, str):
          ...
          if message['type'] == 'progress':
              data = message['data']
              if pbar_ksampler is None:
                pbar_ksampler = tqdm(enumerate(range(data['max'])), total=data['max'], position=2, leave=True, desc="K-Sampler")
              pbar_ksampler.update(1)
...
def prompt_to_image(workflow, positve_prompt, output_path, file_name, negative_prompt='', seed=None, steps=40, cfg=4.2, save_previews=False):
  """
  ...
  Args:
    ...
    output_path (str): The file system path where the generated image(s) should be saved.
    file_name (str): Tthe name of the final image file.
    save_previews (bool, optional): A flag indicating whether to save preview images alongside the final image.
                                    Defaults to False.
  ...
  prompt.get(k_sampler)['inputs']['seed'] = random.randint(10**14, 10**15 - 1) if seed == None else seed
  prompt.get(k_sampler)['inputs']['steps'] = steps
  prompt.get(k_sampler)['inputs']['cfg'] = cfg
  ...
  generate_image_by_prompt(prompt, output_path, file_name, save_previews)
