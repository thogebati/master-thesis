...
class Helper:
    ...
    def _add_avg_depth(self, _dict: dict[str, list[int]], avg_depth:int):
        if Const.avg_depth not in _dict:
            _dict[Const.avg_depth] = [avg_depth]
        else:
            _dict[Const.avg_depth].append(avg_depth)

    def _add_first_position(self, _dict: dict[str, int], pos: str):
        if pos not in _dict:
            _dict[pos] = 1
        else:
            _dict[pos] += 1
            
    def _add_in_front_of_count(self, _dict: dict[str, int], value: str):
        if value not in _dict:
            _dict[value] = 1
        else:
            _dict[value] += 1
            
    def _add_in_front_of_val(self, _dict: dict[str, dict[str, float]], other_value: str, weighted_val: float):
    ...
    def _get_num_faces_top_node(self, num_faces: int):
        # print(f'getting dict for {num_faces} faces')
        # print(json.dumps(General_Node))
        if num_faces >= 5:
