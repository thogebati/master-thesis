...
def run(input_path: str, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
    ...
    ind = splitted.index('face_img')
    date = splitted[ind -1]
    log_dir = os.path.join('results/', f'path_midas_images_{date}.csv')
    
    if os.path.exists(log_dir) or os.path.isfile(log_dir):
        tmp = pd.read_csv(log_dir)
        print("read dataframe")
    else:
        tmp = pd.DataFrame(columns=['image_id', 'path', 'pfm', 'with_negative_prompt'])
        print("new dataframe")
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            if output_path is not None:
                ...
                tmp.loc[len(tmp)] = [image_name.split('/')[-1].split('_')[0], filename + ".pfm", True, False if 'without' in filename else False]
                tmp.loc[len(tmp)] = [image_name.split('/')[-1].split('_')[0], filename + ".png", False, False if 'without' in filename else False]
                tmp.to_csv(log_dir, index=False)
